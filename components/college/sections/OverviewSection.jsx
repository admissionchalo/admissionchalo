...
const SECTION_TAB_LINKS = [
  { test: (t) => /course/i.test(t), slug: "courses", label: "Courses", faqField: "courseFaqs" },
  { test: (t) => /fee/i.test(t), slug: "fees", label: "Fees", faqField: "feesFaqs" },
  { test: (t) => /cutoff|cut-off|cut off/i.test(t), slug: "cutoffs", label: "Cutoff", faqField: "cutoffFaqs" },
  { test: (t) => /admission/i.test(t), slug: "admissions", label: "Admissions", faqField: "admissionsFaqs" },
  { test: (t) => /placement/i.test(t), slug: "placements", label: "Placements", faqField: "placementsFaqs" },
  { test: (t) => /scholarship/i.test(t), slug: "scholarships", label: "Scholarships", faqField: "scholarshipFaqs" },
  { test: (t) => /rank/i.test(t), slug: "overview", label: "Rankings", faqField: null },
  { test: (t) => /campus|infrastructure|facilit/i.test(t), slug: "facilities", label: "Facilities", faqField: "facilitiesFaqs" },
];
...
const inlineLinkStyle = {
  ...
  fontSize: "inherit", fontFamily: "inherit", display: "inline",
};

function highlightIntro(text, phrase, onClick) {
  ...
  if (!match) {
    return (
      <>
        {text}{" "}
        <button onClick={onClick} style={inlineLinkStyle}>{phrase}</button>.
      </>
    );
  }
  ...
  return (
    <>
      {before}
      <button onClick={onClick} style={inlineLinkStyle}>{matched}</button>
      {after}
    </>
  );
}
        ...
        <thead>
          <tr style={{ background: "#f8fafc" }}>
            {table.columns.map((c, i) => (
              <th key={i} style={{ padding: "10px 14px", textAlign: "left", fontSize: 11.5, fontWeight: 700, color: "#374151", textTransform: "uppercase", letterSpacing: "0.03em", borderBottom: "1px solid #e5e7eb" }}>
                {c}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {table.rows.map((row, ri) => (
            <tr key={ri} style={{ background: ri % 2 === 0 ? "#fff" : "#f9fafb", borderBottom: "1px solid #f3f4f6" }}>
              {row.map((cell, ci) => (
                <td key={ci} style={{ padding: "10px 14px", fontSize: 12.5, color: ci === 0 ? "#111827" : "#374151", fontWeight: ci === 0 ? 700 : 400 }}>
                  {cell}
                </td>
              ))}
            </tr>
...
function ArticleSection({ section, data, P, O, onTabLink, defaultOpen = true }) {
  ...
  const sectionFaqs = tabLink?.faqField ? data[tabLink.faqField] : null;
  const phrase = tabLink ? `${data.name} ${tabLink.label}` : null;
  const canNavigate = tabLink && tabLink.slug !== "overview";

  return (
    <section
      id={anchorId}
      style={{
        display: "flex", background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb",
        overflow: "hidden", boxShadow: "0 1px 2px rgba(16,24,40,0.04)", scrollMarginTop: 90,
      }}
    >
      <div style={{ width: 4, flexShrink: 0, background: P }} />

      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, padding: "17px 22px", background: open ? "#fafafa" : "#fff", borderBottom: open ? "1px solid #f0f0f0" : "none" }}>
          {canNavigate ? (
            <button
              onClick={() => onTabLink(tabLink.slug)}
              style={{ background: "none", border: "none", padding: 0, textAlign: "left", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, minWidth: 0 }}
            >
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexShrink: 0, padding: 4 }}
          >
          ...
          <div style={{ padding: "18px 22px 22px" }}>
            {section.intro && (
              <p style={{ fontSize: 13.5, color: "#374151", lineHeight: 1.8, margin: "0 0 18px" }}>
                {phrase ? highlightIntro(section.intro, phrase, () => canNavigate && onTabLink(tabLink.slug)) : section.intro}
              </p>
            )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {section.blocks?.map((block, i) => (
                <div key={i}>
                  {block.heading && (
                    <h3 style={{ fontSize: 14.5, fontWeight: 700, color: "#111827", margin: "0 0 8px" }}>
                      {block.heading}
                    </h3>
                  )}

                  {block.text && (
                    <p style={{ fontSize: 13.5, color: "#374151", lineHeight: 1.75, margin: (block.bullets?.length || block.table) ? "0 0 10px" : 0 }}>
                      {block.text}
                    </p>
                  )}
                  ...
                  {block.bullets?.length > 0 && (
                    <ul style={{ margin: 0, padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: 7 }}>
                      {block.bullets.map((b, bi) => (
                        <li key={bi} style={{ display: "flex", gap: 9, fontSize: 13.5, color: "#374151", lineHeight: 1.65 }}>
                          <span style={{ color: O, fontSize: 15, lineHeight: 1.65, flexShrink: 0 }}>•</span>
                          <span>
                            {typeof b === "string" ? b : (
                              <>
            ...
            {section.note && (
              <aside style={{ marginTop: 18, padding: "10px 14px", borderRadius: 8, background: "#f8fafc", border: "1px solid #eef1f4", display: "flex", gap: 8 }}>
...
export default function OverviewSection({ data }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {hasArticle && (
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb", overflow: "hidden", boxShadow: "0 1px 2px rgba(16,24,40,0.04)" }}>
          <button
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 9, background: `${P}12`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ...
          {tocOpen && (
            <div style={{ padding: "4px 22px 20px", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(230px, 1fr))", gap: 8 }}>
              {data.overviewArticle.map((section, i) => (
                <button
                  key={i}
                  onClick={() => scrollToSection(section.title)}
                  style={{
                    background: "#f8fafc", border: "1px solid #eef1f4", borderRadius: 9,
                    padding: "9px 12px", textAlign: "left", cursor: "pointer",
                    fontSize: 12.5, fontWeight: 600, color: "#2563eb",
                    display: "flex", alignItems: "center", gap: 7,
                  }}
                >
                  <span style={{ width: 5, height: 5, borderRadius: "50%", background: P, flexShrink: 0 }} />
      ...
      {hasFaqs && (
        <section
          ...
          style={{ display: "flex", background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb", overflow: "hidden", boxShadow: "0 1px 2px rgba(16,24,40,0.04)", scrollMarginTop: 90 }}
        >
          <div style={{ width: 4, flexShrink: 0, background: P }} />
          <div style={{ flex: 1, minWidth: 0, padding: "22px" }}>
